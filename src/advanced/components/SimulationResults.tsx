...
import { getLeaderboardAdvanced } from '@/lib/firebaseLeaderboard';
...
export default function SimulationResults({ result, initialInvestment, yearlyAddition = DEFAULT_YEARLY_ADDITION, startYear = DEFAULT_START_YEAR, endYear = DEFAULT_END_YEAR, onReset, benchmarkData, teamName, weeklySeed }: SimulationResultsProps) {
  ...
  // Leaderboard: same as Classic — fetch from Firebase, merge player, display. No AI fallback.
  ...
  useEffect(() => {
    (async () => {
      try {
        const list = await getLeaderboardAdvanced();
        const others = list
          .filter((e) => e.name && e.value > 0)
          .map((e) => ({ name: e.name, value: e.value, returnPct: e.returnPct, isYou: false }));
        const merged = [...others];
        const idx = merged.findIndex((e) => e.name === teamName);
        if (idx >= 0) merged[idx] = { ...playerEntry, isYou: true };
        else merged.push(playerEntry);
        merged.sort((a, b) => b.value - a.value);
        setLeaderboard(merged.slice(0, 50));
      } catch (err) {
        console.warn('Leaderboard fetch failed:', err);
        setLeaderboard([playerEntry]);
      }
    })();
  }, [teamName, finalPortfolioValue, yourReturnPct]);
